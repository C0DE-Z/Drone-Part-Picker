...
export class WebCrawlerService {
    ...
    async extractLinksFromPage(page, config) {
    ...
    determineCategory(url, config, productName, description) {
    ...
    determineCategoryByScoring(productName, description) {
        ...
        // Use rule-based classification instead of fuzzy scoring
        const category = this.classifyByRules(textToAnalyze);
    ...
    classifyByRules(text) {
        ...
        if (this.isDefinitelyBattery(text))
            return 'battery';
        if (this.isDefinitelyProp(text))
            return 'prop';
        if (this.isDefinitelyFrame(text))
            return 'frame';
        if (this.isDefinitelyCamera(text))
            return 'camera';
        // Step 2: Check for ESC/Stack classification (most complex)
        const escStackResult = this.classifyEscStack(text);
        if (escStackResult)
            return escStackResult;
        // Step 3: Check for motor/power system
        if (this.isDefinitelyMotor(text))
            return 'motor';
    ...
    isDefinitelyMotor(text) {
        ...
        if (text.includes('power system')) {
            console.log(`✅ Motor: power system`);
            return true;
        }
    ...
    fallbackClassification(text) {
        ...
        if (text.includes('kv'))
            scores.motor += 2;
        if (text.includes('brushless'))
            scores.motor += 1;
        if (text.includes('stator'))
            scores.motor += 1;
        // Prop indicators
        if (text.includes('prop'))
            scores.prop += 3;
        if (text.includes('blade'))
            scores.prop += 2;
        if (/\d+x\d+/.test(text))
            scores.prop += 1;
        // Battery indicators
    ...
    extractSpecifications($, selector) {
        const specs = {};
    ...
    extractAllSpecifications($, name, description, category) {
        const specs = {};
        ...
        switch (category) {
            ...
            case 'prop':
                ...
                if (hubMatch)
                    specs.hubDiameter = `${hubMatch[1]}mm`;
                break;
            case 'frame':
            ...
            case 'camera':
                ...
                if (sensorMatch)
                    specs.sensor = sensorMatch[1] || `${sensorMatch[2]}mm`;
    ...
    async crawlVendor(vendorName, maxPages = 500) {
        ...
        for (const queueItem of collectionQueue.slice(0, maxCollectionPages)) {
            if (this.visitedUrls.has(queueItem.url))
                continue;
    ...
    extractMotorSpecs($, name, description) {
        const specs = {};
        const textContent = `${name} ${description}`.toLowerCase();
    ...
    async crawlAllVendors(maxPagesPerVendor = 500) {
        const allProducts = [];
