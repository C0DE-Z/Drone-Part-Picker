// Physics-based drone performance analysis services
...
// Part Classification Service
export class PartClassificationService {
...
export class PerformancePredictor {
  static async predictPerformance(parts: CustomPart[]): Promise<{
    ...
    // Apply user feedback calibration
    const calibrationFactors = FeedbackService.getCalibrationFactors();
    const calibratedFlightTime = flightTime * calibrationFactors.flightTimeMultiplier;
    const calibratedTopSpeed = practicalSpeed * calibrationFactors.topSpeedMultiplier;
    const calibratedPowerConsumption = totalPowerConsumption * calibrationFactors.powerConsumptionMultiplier;
    const calibratedThrustToWeight = thrustToWeight * calibrationFactors.thrustToWeightMultiplier;
    
    // Generate prediction ID for feedback tracking
    const predictionId = FeedbackService.generatePredictionId();
    
    return {
      predictionId,
      estimatedPerformance: {
        thrust: Math.round(totalThrust),
        flightTime: Math.round(calibratedFlightTime * 10) / 10,
        topSpeed: Math.round(calibratedTopSpeed),
        powerConsumption: Math.round(calibratedPowerConsumption),
        thrustToWeight: Math.round(calibratedThrustToWeight * 100) / 100,
        totalWeight: Math.round(estimatedWeight),
        maxTheoreticalSpeed: Math.round(maxSpeed),
        powerToWeight: Math.round((calibratedPowerConsumption / (estimatedWeight / 1000)) * 10) / 10,
      },
      confidence,
      limitations,
      recommendations,
...
export class FeedbackIntegrationService {
  private static feedbackData: Array<{
    predictionId: string;
  ...
  static getCalibrationFactors(): {
    ...
    // These factors are updated based on user feedback
    // Initially conservative estimates, improved over time
    return {
      flightTimeMultiplier: 0.85, // Real flight time is typically 85% of prediction
      speedMultiplier: 0.92, // Real speed is typically 92% of prediction
      powerMultiplier: 1.1, // Real power consumption is typically 110% of prediction
    };
  }
}
